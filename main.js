...
function resetBillboard(group, first = false) {
    ...
    const z = first ? (camera.position.z - (60 + Math.random() * 260)) : (camera.position.z - (120 + Math.random() * 240));
...
for (let i = 0; i < LAMP_COUNT; i++) {
    ...
    L.position.set((Math.random() < 0.5 ? -1 : 1) * (8 + Math.random() * 6), 0, camera.position.z - (60 + Math.random() * 160));
    scene.add(L); lampPool.push(L);
}
function resetLamp(L) {
    L.position.set((Math.random() < 0.5 ? -1 : 1) * (8 + Math.random() * 6), 0, camera.position.z - (80 + Math.random() * 180));
...
function resetVine(v) {
    v.position.set((Math.random() < 0.5 ? -1 : 1) * (7.5 + Math.random() * 5.5), 0, camera.position.z - (70 + Math.random() * 180));
...
function placeRift(r, first = false) {
    ...
    const y = .6 + Math.random() * 1.6; const z = first ? (camera.position.z - (70 + Math.random() * 160)) : (camera.position.z - (100 + Math.random() * 200));
...
const GAME_DURATION_MS = 10 * 1000; // 120 seconds
...
function startCountdownAndGame() {
    ...
    countdownInterval = setInterval(() => {
        ...
        if (countdownEl) countdownEl.innerText = countdownValue > 0 ? countdownValue : "GO!";
...
function loadPlayer() {
    fbxLoader.load('./models/player.fbx', (object) => {
        ...
        player.position.set(PLAYER_LANE_X, 1, 10);
...
let sessionTyped = 0, sessionWrong = 0, sessionStart = null;
...
function updatePlayer0Speed(dt) {
    ...
    if (targetSpeed === 0 && player0Speed > 0) {
        ...
        const decelPerSecond = player0Speed / (SPEED_STOP_TIME + player0Speed * 0.1);
...
function showFinalLeaderboard() {
    ...
    overlay.innerHTML = `<h3 style="margin:0 0 8px 0">Final Standings</h3>` + standings.map((s, i) => `<div style="padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.03)"><strong>#${i + 1}</strong> ${s["Distance Covered"]}m</div>`).join('') + '<div style="margin-top:10px;text-align:center"><button id="closeFinal" class="btn">Close</button></div>';
...
function tick(now) {
    ...
    if (gameTimerRunning && countdownEl) {
        ...
        if (timeLeft <= 10000) countdownEl.style.color = '#ffd54f'; else countdownEl.style.color = 'red';
        if (timeLeft <= 0) {
            finishGameByTime();
        }
    }
